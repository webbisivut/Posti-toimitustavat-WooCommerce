 ...
 * @return array
 */
class posti_postipaketti_cart_items_sz {
...
/**
 * WB_Posti_Nouto_Shipping_Method_sz Class
 *
 * @class WB_Posti_Nouto_Shipping_Method_sz
 * @version	1.0.0
 * @since 1.0.0
 * @package	WB_Posti_Toimitustavat
 * @author Webbisivut.org
 */
function WB_Posti_Nouto_Shipping_Method_sz_Init() {

	if ( ! class_exists( 'WB_Posti_Nouto_Shipping_Method_sz' ) ) {

		class WB_Posti_Nouto_Shipping_Method_sz extends WC_Shipping_Method {
			...
			*/
			public function __construct( $instance_id = 0 ) {
				$this->id = 'wb_posti_nouto_shipping_method_sz'; // Id for your shipping method. Should be uunique.
				$this->instance_id = absint( $instance_id );
				$this->method_title = __( 'Posti Postipaketti (Posti Toimitustavat)', 'wb-posti' ); // Title shown in admin
				$this->method_description = __( 'Postipaketti toimitustapa', 'wb-posti' ); // Description shown in admin
				$this->supports = array(
					'shipping-zones',
					'instance-settings',
				);
			...
			*/
			function init() {
				...
				$this->posti_paino1		   = str_replace(",", ".", esc_attr( $this->get_option('posti_paino1') ));
				$this->posti_hinta1		   = str_replace(",", ".", esc_attr( $this->get_option('posti_hinta1') ));

				$this->posti_paino2		   = str_replace(",", ".", esc_attr( $this->get_option('posti_paino2') ));
				$this->posti_hinta2		   = str_replace(",", ".", esc_attr( $this->get_option('posti_hinta2') ));

				$this->posti_paino3		   = str_replace(",", ".", esc_attr( $this->get_option('posti_paino3') ));
				$this->posti_hinta3		   = str_replace(",", ".", esc_attr( $this->get_option('posti_hinta3') ));

				$this->posti_paino4		   = str_replace(",", ".", esc_attr( $this->get_option('posti_paino4') ));
				$this->posti_hinta4		   = str_replace(",", ".", esc_attr( $this->get_option('posti_hinta4') ));

				$this->posti_paino5		   = str_replace(",", ".", esc_attr( $this->get_option('posti_paino5') ));
				$this->posti_hinta5		   = str_replace(",", ".", esc_attr( $this->get_option('posti_hinta5') ));

				$this->posti_ilm_toim	   = str_replace(",", ".", esc_attr( $this->get_option('posti_ilm_toim') ));
				$this->posti_kas_kulut	   = str_replace(",", ".", esc_attr( $this->get_option('posti_kas_kulut') ));

				$this->posti_max_korkeus   = str_replace(",", ".", esc_attr( $this->get_option('posti_max_korkeus') ));
				$this->posti_max_pituus    = str_replace(",", ".", esc_attr( $this->get_option('posti_max_pituus') ));
				$this->posti_max_leveys    = str_replace(",", ".", esc_attr( $this->get_option('posti_max_leveys') ));

				$this->posti_max_paino	   = str_replace(",", ".", esc_attr( $this->get_option('posti_max_paino') ));

				$this->posti_kuponki	   = esc_attr( $this->get_option('posti_kuponki') );
			...
			function init_form_fields() {
				...
				$this->instance_form_fields = array(
					...
					'posti_max_paino'	   => array(
						'title'            => __('Postipaketti maksimi paino', 'wb-posti'),
						'type'             => 'text',
						'placeholder'	   => '30',
					...
					'posti_ilm_toim'	   => array(
						'title'            => __('Posti-Paketti ilmaisen toimituksen raja', 'wb-posti'),
						'type'             => 'text',
						'placeholder'	   => '100',
			...
			*/
			public function calculate_shipping( $package = array() ) {
				...
				$paino1_korkeus = $this->posti_paino1;
				$paino2_korkeus = $this->posti_paino2;
				$paino3_korkeus = $this->posti_paino3;
				$paino4_korkeus = $this->posti_paino4;
				$paino5_korkeus = $this->posti_paino5;

				// Katsotaan kummanko yksikön mukaan hinnoitellaan
				$korkeus_vai_paino = $this->korkeus_vai_paino;
				
				$kassan_tiedot = new posti_postipaketti_cart_items_sz();
				...
				$ilm_toim = $this->posti_ilm_toim;

				// Kuponki
				$has_coupon = false;
				$all_coupons = array();
				$annettu_koodi = $this->posti_kuponki;
				...
				if ( ($ilm_toim !='') && ($ilm_toim <= floatval($cart_price)) OR $has_coupon ) {
					...
					if ( (0 <= $yksikko) && ($yksikko <= $paino1_korkeus) ) {
						$lopullinen_hinta = $this->posti_hinta1 + $this->posti_kas_kulut;
					} elseif ( ($paino1_korkeus <= $yksikko) && ($yksikko <= $paino2_korkeus) ) {
						$lopullinen_hinta = $this->posti_hinta2 + $this->posti_kas_kulut;
					} elseif ( ($paino2_korkeus <= $yksikko) && ($yksikko <= $paino3_korkeus) ) {
						$lopullinen_hinta = $this->posti_hinta3 + $this->posti_kas_kulut;
					} elseif ( ($paino3_korkeus <= $yksikko) && ($yksikko <= $paino4_korkeus) ) {
						$lopullinen_hinta = $this->posti_hinta4 + $this->posti_kas_kulut;
					} elseif ( ($paino4_korkeus <= $yksikko) && ($yksikko <= $paino5_korkeus) ) {
						$lopullinen_hinta = $this->posti_hinta5 + $this->posti_kas_kulut;
					} else {
						$lopullinen_hinta = $this->posti_hinta5 + $this->posti_kas_kulut;
					}
				}

				$rate = apply_filters('wb_posti_paketti_rate_filter', array(
...
add_action( 'woocommerce_shipping_init', 'WB_Posti_Nouto_Shipping_Method_sz_init' );

function add_WB_Posti_Nouto_Shipping_Method_sz( $methods ) {

	$methods['wb_posti_nouto_shipping_method_sz'] = 'WB_Posti_Nouto_Shipping_Method_sz';
	return $methods;

}

add_filter( 'woocommerce_shipping_methods', 'add_WB_Posti_Nouto_Shipping_Method_sz' );
...
* @return void
*/
function hide_show_wb_posti_sz( $rates, $package ) {
	$woocommerce = function_exists('WC') ? WC() : $GLOBALS['woocommerce'];
	...
	if(isset($rates)) {
		...
		foreach ($shippingIds as $shipping_id) {
			if (strpos($shipping_id, 'wb_posti_nouto_shipping_method_sz') !== false) {
				$get_the_id = str_replace('wb_posti_nouto_shipping_method_sz', '', $shipping_id);
		...
		$posti_shipping_method = new WB_Posti_Nouto_Shipping_Method_sz( $instance_id = $get_the_id );

		$max_paino = $posti_shipping_method->posti_max_paino;

		if($posti_shipping_method->posti_max_korkeus != '') {
			$max_korkeus = $posti_shipping_method->posti_max_korkeus;
		} else {
			$max_korkeus = 60;
		}

		if($posti_shipping_method->posti_max_leveys != '') {
			$max_leveys = $posti_shipping_method->posti_max_leveys;
		} else {
			$max_leveys = 60;
		}

		if($posti_shipping_method->posti_max_pituus != '') {
			$max_pituus = $posti_shipping_method->posti_max_pituus;
		} else {
			$max_pituus = 100;
		}
		...
		if( max($height) > $max_korkeus OR max($length) > $max_pituus OR max($width) > $max_leveys OR max($weight) > $max_paino ) {
			...
			foreach ( $rates as $rate_id => $rate ) {
				if ( 'wb_posti_nouto_shipping_method_sz' !== $rate->method_id ) {
...
add_filter( 'woocommerce_package_rates', 'hide_show_wb_posti_sz' , 10, 2 );
